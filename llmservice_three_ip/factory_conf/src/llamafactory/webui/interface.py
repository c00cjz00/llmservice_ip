...
def run_web_ui() -> None:
    ...
    create_ui().queue().launch(share=gradio_share, server_name=server_name, server_port=server_port,auth=(gradio_user, gradio_password), inbrowser=True)

def run_web_demo() -> None:
    server_name = os.environ.get("GRADIO_SERVER_NAME", "0.0.0.0")
    server_port = int(os.environ.get("GRADIO_SERVER_PORT", "7860"))
    gradio_share = os.environ.get("GRADIO_SHARE", "0").lower() in ["true", "1"]
    create_web_demo().queue().launch(share=gradio_share, server_name=server_name, server_port=server_port, inbrowser=True)
